...
/**
 * Kana first, then kanji by JLPT level. Within a kanji level the order is
 * newspaper frequency, so the characters that carry the most text come first
 * — learning 日 before 妹 is worth more than any amount of tidiness.
 */
export const LEVELS: Level[] = [
  ...KANA_LEVELS.map((l) => ({
    ...l,
    cards: DECKS.find((d) => d.id === l.id)!.cards as DrillCard[],
  })),
  { id: 'kanji-n5', label: 'Кандзи N5', note: 'первые 79 знаков, по частоте в печати', cards: kanjiCards(5) },
